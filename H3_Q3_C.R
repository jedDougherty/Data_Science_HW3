...
### create a data frame to do the following analysis for one time
my.data <- data.frame(xom.ts.return, xom.ts.volume)
...
#Create a function to train the linear regression model
require(dyn)
train.function <- function(train.set){
    train.result <- dyn$lm(train.set ~ lag(train.set, -1) + lag(train.set, -2))
}

#Creat a function to do the exponential decay 
my.exponential.decay.function <- function(xom.ts.data){  # When call the function at the first time, xom.ts.data is the the log.return 
                                       # When call the function at the second time, xom.ts.data is the volume data
    #browser()
    print("Now we are in the exponential decay function")
    #Calculate old volatility.
    xom.old <- as.matrix(xom.ts.data[1:365]) #Using 1-year lookback window
    ...
    for (i in 366:length(xom.ts.data)) {
        vol.old[i] <- volatility.new(xom.ts.data[i], vol.old[i - 1], decay)
    }
    ...
    xom.ts.data_new <- xom.ts.data / sqrt(vol.old)
    xom.ts.data_new <- zoo(xom.ts.data_new, exxon$Date)
    plot(xom.ts.data_new)
    
    acf(xom.ts.data_new, na.action=na.pass) #Lags 2-7 are significant!
    pacf(xom.ts.data_new, na.action=na.pass) #Lags 2-7 are significant!
    
    return(xom.ts.data_new)
}
    

#Create a function to do the linear regression    
my.linear.regression.function <- function(xom.ts.data){
    
    print("Now we are in the linear regression function")
    
    xom.lm.train <- xom.ts.data[1 : 5803]  #using the previous half data as the training set
    xom.lm.test <- xom.ts.data[5804 : 10803]  ##using the other half data as the testing set
...
##############################
#Call the functions above to draw the cumulative PnL plot for log.return and volume data respectively
###############################

for(i in 1:2){
    
    print(paste(c("The data we are using now is ", names(my.data[i]))))
    
    #Do exponential decay of the log.return and volume data respectively
    data.with.exp.decay <- my.exponential.decay.function(my.data[, i])  #Use the my.function to calculate the result of log.return and volume data respectively
    
    #Do linear regression function
    test.pnl <- my.linear.regression.function(data.with.exp.decay)
    
    #Draw the cumulatice PnL plot
